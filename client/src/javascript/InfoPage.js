...
function InfoPage() {
  ...
  useEffect(() => {
    async function fetchMovie() {
      try {
        const { data } = await axios.get(`https://api.themoviedb.org/3/movie/${number}?api_key=${API_KEY}&language=en-US`);
        setMovie(data);
      } catch (error) {
        console.error(error);
      }
  ...
  const { title, name, original_name, backdrop_path, genres, vote_average, runtime, overview } = movie;
